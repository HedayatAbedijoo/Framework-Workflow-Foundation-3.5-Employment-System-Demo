...
namespace WFTools.Services.Tracking
{
    ...
    /// 
    /// An <see cref="IResourceProvider" /> and <see cref="ITrackingChannelResourceAccessor" /> 
    /// work hand-in-hand to actually persist workflow tracking information to the tracking store.
    /// </summary>
    public abstract class GenericTrackingChannel : TrackingChannel, IPendingWork
    {
        ...
        /// An <see cref="IResourceProvider" /> responsible for providing
        ...
        /// is associated.
        /// </summary>
        private WorkflowInstanceSummary _workflowInstanceSummary;
        ...
        /// </param>
        /// <returns>
        /// An <see cref="ITrackingChannelResourceAccessor" /> responsible for
        /// manipulating the underlying tracking store.
        /// </returns>
        ...
        public void Commit(Transaction transaction, ICollection items)
        {
            ...
            try
            {
                using (ITrackingChannelResourceAccessor resourceAccessor = CreateAccessor(_resourceProvider, transaction))
                {
                    if (_workflowInstanceSummary == null)
                    {
                        _workflowInstanceSummary =
                            resourceAccessor.InsertOrGetWorkflowInstance(
                                buildWorkflowInstanceSummary());
                    }
                    ...
                    if (activityTrackingRecords.Count > 0)
                        resourceAccessor.InsertTrackingRecords(_workflowInstanceSummary, activityTrackingRecords);

                    if (userTrackingRecords.Count > 0)
                        resourceAccessor.InsertTrackingRecords(_workflowInstanceSummary, userTrackingRecords);

                    if (workflowTrackingRecords.Count > 0)
                        resourceAccessor.InsertTrackingRecords(_workflowInstanceSummary, workflowTrackingRecords);

                    if (workflowChangeRecords.Count > 0)
                        resourceAccessor.InsertTrackingRecords(_workflowInstanceSummary, workflowChangeRecords);
                }
            }
            catch (Exception e)
            {
        ...
        /// <summary>
        /// Returns a nullable <see cref="Guid" /> if the specified value is 
        /// <see cref="Guid.Empty" />.
        /// </summary>
        /// <param name="guid">
        /// A <see cref="Guid" /> to convert to a nullable <see cref="Guid" />.
        /// </param>
        /// <returns>
        /// <c>null</c> if <paramref name="guid"/> is equal to <see cref="Guid.Empty" />
        /// otherwise returns <paramref name="guid"/> as a nullable <see cref="Guid" />.
        /// </returns>
        private static Guid? buildNullableGuid(Guid guid)
        {
            if (guid == Guid.Empty)
                return null;
        ...
        {
            IList<String> callPathList = _trackingParameters.CallPath;
        ...
        private ActivitySummary buildRootActivitySummary()
        {
            ...
            activityQueue.Enqueue(_trackingParameters.RootActivity);
        ...
        /// </param>
        /// <returns>
        /// A <see cref="SerialisableActivityTrackingRecord" /> containing
        /// the serialised data.
        /// </returns>
        ...
        /// </param>
        /// <returns>
        /// A <see cref="SerialisableUserTrackingRecord" /> of containing
        /// the serialised data.
        /// </returns>
        ...
        /// </param>
        /// <returns>
        /// A <see cref="SerialisableWorkflowChangeRecord" /> containing the
        /// serialised data.
        /// </returns>
        ...
        /// <summary>
        /// Serialise a <see cref="RemovedActivityAction" /> ready for persistence.
        /// </summary>
        /// <param name="action">
        /// The original <see cref="RemovedActivityAction" /> to serialise.
        /// </param>
        /// <param name="order">
        /// Indicates the order the activity was added.
        /// </param>
        /// <returns>
        /// A <see cref="SerialisableActivityRemovedAction" /> containing a serialised
        /// copy of the <see cref="RemovedActivityAction" />.
        /// </returns>
