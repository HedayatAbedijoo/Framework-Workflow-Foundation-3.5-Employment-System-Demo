...
namespace WFTools.Services.Tracking
{
    ...
    /// 
    /// An <see cref="IResourceProvider" /> and <see cref="ITrackingServiceResourceAccessor" /> 
    ...
    public abstract class GenericTrackingService : TrackingService, IProfileNotification, IDisposable
    {
        ...
        /// </summary>
        protected GenericTrackingService() : this(_defaultProfileChangeInterval) { }
        /// <summary>
        ...
        {
            _profileChangeInterval = profileChangeInterval;
        }

        /// <summary>
        /// Default timespan of 60 seconds to poll for profile changes.
        /// </summary>
        private static readonly TimeSpan _defaultProfileChangeInterval = TimeSpan.FromSeconds(60);

        /// <summary>
        /// The last time that profile changes were checked for.
        /// </summary>
        private DateTime _lastProfileChangeCheck = DateTime.UtcNow;
        /// <summary>
        /// Interval in which to poll for profile changes.
        /// </summary>
        private TimeSpan _profileChangeInterval;

        /// <summary>
        /// Timer used to poll for profile changes.
        /// </summary>
        private Timer _profileChangeTimer;
        ...
        protected override void Start()
        {
            ...
            try
            {
                ...
                _profileChangeTimer = new Timer(_profileChangeInterval.TotalMilliseconds);
                _profileChangeTimer.AutoReset = false;
                _profileChangeTimer.Elapsed += profileChangeTimer_Elapsed;
                _profileChangeTimer.Start();
            ...
            catch (Exception e)
            {
                ...
                RaiseServicesExceptionNotHandledEvent(trackingException, Guid.Empty);
        ...
        protected override void Stop()
        {
            ...
            try
            {
                if (_profileChangeTimer != null)
                {
                    _profileChangeTimer.Dispose();
                    _profileChangeTimer = null;
                }
        ...
        private void profileChangeTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            try
            {
                ...
                {
                    IList<TrackingProfileChange> changes = GetTrackingProfileChanges(ref _lastProfileChangeCheck);
        ...
        protected virtual IList<TrackingProfileChange> GetTrackingProfileChanges(ref DateTime lastCheck)
        {
            ...
            try
            {
                using (ITrackingServiceResourceAccessor resourceAccessor = CreateAccessor(_resourceProvider))
                {
        ...
        protected override TrackingProfile GetProfile(Guid workflowInstanceId)
        {
            ...
            catch (Exception e)
            {
                ...
                RaiseServicesExceptionNotHandledEvent(trackingException, workflowInstanceId);
