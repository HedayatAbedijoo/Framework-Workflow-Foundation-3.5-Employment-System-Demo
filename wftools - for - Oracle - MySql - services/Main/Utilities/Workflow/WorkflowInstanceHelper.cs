...
namespace WFTools.Utilities.Workflow
{
    ...
    public static class WorkflowInstanceHelper
    {
        ...
        /// <summary>
        /// Create a serialised representation of the specified workflow activity.
        /// </summary>
        /// <param name="activityToSerialise">
        /// <see cref="Activity" /> to generate a XOML document from.
        /// </param>
        /// <returns>
        /// String containing the XOML document for the specified activity.
        /// </returns>
        public static String SerialiseActivity(Activity activityToSerialise)
        {
            using (StringWriter stringWriter = new StringWriter(CultureInfo.InvariantCulture))
            {
                using (XmlWriter xmlWriter = createXmlWriter(stringWriter))
                {
                    new WorkflowMarkupSerializer().Serialize(xmlWriter, activityToSerialise);
        ...
        /// <summary>
        /// Create an <see cref="Activity" /> from the specified serialised representation.
        /// </summary>
        /// <param name="serialisedActivity">
        /// Serialised representation of an activity.
        /// </param>
        /// <returns>
        /// An <see cref="Activity" />.
        /// </returns>
        public static Activity DeserialiseActivity(String serialisedActivity)
        {
            using (StringReader stringReader = new StringReader(serialisedActivity))
            {
                using (XmlReader xmlReader = createXmlReader(stringReader))
                {
                    return (Activity)new WorkflowMarkupSerializer().Deserialize(xmlReader);
                }
            }
        }
