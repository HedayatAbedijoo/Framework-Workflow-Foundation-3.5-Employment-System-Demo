...
CREATE OR REPLACE PACKAGE BODY WORKFLOW_PERSISTENCE_PKG AS
	PROCEDURE GetOwnershipResult
	(
		p_INSTANCE_ID IN INSTANCE_STATE.INSTANCE_ID%TYPE
		,p_OWNER_ID OUT INSTANCE_STATE.OWNER_ID%TYPE
		,p_RESULT OUT NUMBER
	)
	AS
	BEGIN
	...
	AS
		CURSOR l_WORKFLOW_IDS IS
			...
			WHERE
				...
				AND (OWNER_ID IS NULL OR OWNED_UNTIL < SYS_EXTRACT_UTC(SYSTIMESTAMP))
			FOR UPDATE;
	BEGIN
		FOR l_INSTANCE_STATE IN l_WORKFLOW_IDS
		LOOP
			IF l_WORKFLOW_IDS%NOTFOUND THEN
				EXIT;
			ELSE
				UPDATE INSTANCE_STATE
				SET
		...
		OPEN p_WORKFLOW_IDS FOR
			...
			WHERE
				...
				AND (
					...
					OR (OWNED_UNTIL < SYS_EXTRACT_UTC(SYSTIMESTAMP))
				);
	END;
